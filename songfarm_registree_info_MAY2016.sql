...
--
-- Host: localhost:3306
-- Generation Time: May 24, 2016 at 07:23 PM
...
CREATE TABLE IF NOT EXISTS `user_register` (
  ...
  PRIMARY KEY (`user_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Contains Registree Info for users who register at Songfarm.ca' AUTO_INCREMENT=83 ;
...
CREATE TABLE IF NOT EXISTS `user_timezone` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Stores user''s selected timezone' AUTO_INCREMENT=65 ;
...
ALTER TABLE `songcircle_register`
  ...
  ADD CONSTRAINT `songcircle_register_ibfk_2` FOREIGN KEY (`songcircle_id`) REFERENCES `songcircle_create` (`songcircle_id`) ON DELETE CASCADE;
